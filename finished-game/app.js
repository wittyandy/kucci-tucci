...
const categories=[['all','✦'],['seats','🛋️'],['beds','🛏️'],['tables','🪑'],['decor','🌿'],['friends','☺'],['pets','🐾']];
...
let state={items:[],scene:'city',time:'day',photo:null,roomName:'My Happy Room'};
let history=[],selected=null,activeCat='all',maker={type:'sofa',color:colors[0]};
const artTypes=new Set(['sofa','bed','table','lamp','plant','rug','poster','cat','dog','rabbit','turtle','sofa-blue','sofa-floral','bed-canopy','bed-modern','table-desk','table-coffee','chair-shell','chair-bean','books-stack','books-basket','toy-bear','toy-blocks']);
const artFilters={'#e98983':'none','#87c9b4':'hue-rotate(105deg) saturate(.72)','#c3a4df':'hue-rotate(255deg) saturate(.72)','#efbd5f':'hue-rotate(315deg) saturate(.92)','#78a9d0':'hue-rotate(155deg) saturate(.72)','#d69abd':'hue-rotate(285deg) saturate(.7)'};
const artBaseColors={sofa:'#e98983',bed:'#c3a4df',table:'#d78f69',lamp:'#f1c85e',plant:'#70b78b',rug:'#d69abd',poster:'#ed918a'};

function furnitureHTML(type,color,facing='front'){
  const source=facing==='front'?`assets/art/${type}.webp`:`assets/art/directions/${type}-${facing}.webp`;
  if(type==='kucci'||type==='tucci') return `<div class="furniture art character-art ${type}"><img src="${source}" alt="" draggable="false"></div>`;
  if(artTypes.has(type)){const filter=artBaseColors[type]&&color!==artBaseColors[type]?(artFilters[color]||'none'):'none';return `<div class="furniture art ${type}" style="--art-filter:${filter}"><img src="${source}" alt="" draggable="false"></div>`}
...
function snapshotAssetKey(src){const match=String(src).replace(/\\/g,'/').match(/assets\/art\/[^?#]+\.webp/i);return match?match[0]:String(src)}
...
async function exportRoomImage(){
  ...
  try{
    ...
    for(const item of [...state.items].sort((a,b)=>a.z-b.z)){
      ...
      ctx.save();ctx.translate(item.x/100*canvas.width,item.y/100*canvas.height);ctx.rotate((item.rotation||0)*Math.PI/180);try{ctx.filter=getComputedStyle(sprite).filter}catch(error){}ctx.drawImage(img,-w/2,-h/2,w,h);ctx.restore();
    }
...
function modify(action){const item=getSelected();if(!item)return;snapshot();if(action==='delete'){playFx('trash');state.items=state.items.filter(x=>x.uid!==selected);selected=null}else if(action==='duplicate'){playFx('copy');addItem({...item,uid:undefined},Math.min(92,item.x+5),Math.min(91,item.y+5));return}else if(action==='rotate'){playFx('rotate');item.rotation=(item.rotation+15)%360}else if(action==='rotate-left'){playFx('rotate');item.rotation=(item.rotation-15+360)%360}else if(action==='face-west'){playFx('direction');item.facing='west'}else if(action==='face-east'){playFx('direction');item.facing='east'}else if(action==='face-forward'){playFx('direction');item.facing='front'}else if(action==='front'){playFx('layer');item.z=Math.max(0,...state.items.map(x=>x.z))+1}else if(action==='back'){playFx('layer');item.z=Math.min(...state.items.map(x=>x.z))-1}renderPlaced();showSelection();}
...
document.addEventListener('pointerdown',e=>{if(!e.target.closest('.placed,#selectionBar'))clearSelection()},{capture:true});
...
document.addEventListener('click',e=>{if(e.target.closest('button')&&!e.target.closest('#playBtn,#soundBtn,#sceneBtn,#timeBtn,#clearBtn,#snapshotBtn,#exportBtn,[data-action],[data-scene]'))playFx('click')},{capture:true});
